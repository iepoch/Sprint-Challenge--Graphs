...
def bfs():
    ...
    visited = set()  # This will be a dictionary and not a set by the time I am done

    # adding the players current room to the queue
    queue.enqueue([player.currentRoom.id])
    # we know are size will always be larger than 0
    while queue.size() > 0:
        # dequeue onto the path
        path = queue.dequeue()
        # we want to get the last node from the path
        node = path[-1]
...
while len(maze) < len(roomGraph):
    ...
    # Lets take the direct path of the room
    direct_path = player.currentRoom.getRoomInDirection
    # all new exits we find
    new_exits = []
    # for each direction in the current exited rooms
    for direction in cur_exits:
        # if the curent exit direction is ?
        if cur_exits[direction] == '?':
            # lets append that new direction
            new_exits.append(direction)
            # if the new exits length is greater than 0
        if len(new_exits) > 0:
            ...
            # have the player move to the new direction
            player.travel(direction)
            # Append this new direction to our path we travled.
